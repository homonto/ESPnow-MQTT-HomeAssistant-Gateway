...
// sensors related data and functions
// publish all data coming from sensors
bool mqtt_publish_data()
{
  ...
  // {
    ...
    Serial.print("\thostname=");Serial.println(myData.host);
    Serial.print("\tSize of message=");Serial.print(sizeof(myData));Serial.println(" bytes");
  ...
  Serial.print("sending ["+ String(myData.host) + "] data to HA...........");
  if (debug_mode) Serial.println();
  publish_status = mqtt_publish_sensors_values(myData.host);
  if (publish_status)
  {
  ...
  // update "last GW status sensor"
  Serial.print("[last_updated_sensor] updating GW on HA...");
  if (debug_mode) Serial.println();
  publish_status = mqtt_publish_gw_last_updated_sensor_values(myData.host);
  if (publish_status)
  {
    Serial.println(".SUCCESSFULL");
  } else
  ...
  long em = millis();
  long tt = em - sm;
  Serial.println("[mqtt_publish_data] took "+ String(em - sm)+"ms\n");
